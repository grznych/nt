...
\section{Признаки делимости}
	...
	Чтобы определить делится ли число $A\in\mathbb{N}$ на другое число \mbox{$m\in\mathbb{N}$}, будем строить последовательность натуральных чисел $(A_i)_{i=0}^n$ такую, что
	%, где $A_0=A$, с помощью некоторой функции $f\colon\mathbb{N}\to\mathbb{N}$ такой, что
	\begin{enumerate}
		\item $A_0=A$,
		\item существует некоторая функция $f$ такая, что $f(A_i)=A_{i+1}$,
		\item $A_i>A_{i+1}$ для любого $i\in\mathbb{N}_{n-1}$,
		\item $0\leqslant A_n<m$,
		\item $A\divisible m$ тогда и только тогда, когда $A_i\divisible m$ для любого $i\in\mathbb{N}_n$.
	\end{enumerate}
